...
def plot_hist(expts, param, thresholds=None):
    ...
    if thresholds is not None:
        ax.axvline(thresholds["opt"]["threshold"], color='gray', linestyle='--', label=f'opt at {thresholds["opt"]["threshold"]}')
        ax.axvline(thresholds["mid"]["threshold"], color='gray', linestyle='--', label=f'mid at {thresholds["mid"]["threshold"]}')
...
def bundle_sep_expt(D=256, M=1000, K=10, n_trials=1000, plot=True):
